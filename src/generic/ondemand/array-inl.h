...
simdjson_really_inline bool array::operator!=(const array &) noexcept {
  ...
  if (at_start) {
    at_start = false;
    if (doc->iter.is_empty_array()) {
      logger::log_value(doc->iter, "empty array", "", -2);
      return false;
    }
    logger::log_start_value(doc->iter, "array", -1, -1);
    return true;
  }
  return !finished();
}
simdjson_really_inline array &array::operator++() noexcept {
  ...
  bool has_next;
  if ((error = doc->iter.next_element(container).get(has_next))) {
    return *this;
  }
  if (!has_next) { logger::log_end_value(doc->iter, "array"); }
  return *this;
}
