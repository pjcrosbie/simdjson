...
simdjson_really_inline bool object::check_empty_object() noexcept {
  at_start = false;
  if (doc->iter.is_empty_object()) {
    logger::log_value(doc->iter, "empty object", "", -2);
    return false;
  }
...
simdjson_really_inline simdjson_result<value> object::operator[](const std::string_view key) noexcept {
  ...
  logger::log_end_value(doc->iter, "object");
...
simdjson_really_inline object &object::operator++() noexcept {
  ...
  bool has_next;
  if ((error = doc->iter.next_field(container).get(has_next) )) { return *this; }
  if (!has_next) { logger::log_end_value(doc->iter, "object"); }
  return *this;
}
